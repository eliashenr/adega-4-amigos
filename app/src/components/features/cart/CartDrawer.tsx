...
function buildWhatsAppMessage(
  ...
  const msg = [
    `*Pedido${orderId ? ` #${orderId}` : ''} - ${STORE.name}*`,
    '',
    ...lines,
    '',
    ...
    `*Entrega:* A combinar`,
    '',
    'Ola! Gostaria de fazer este pedido.',
  ].join('\n');
...
export function CartDrawer({ open, onClose }: CartDrawerProps) {
  ...
  async function handleWhatsApp() {
    ...
    try {
      // Persist order to database before opening WhatsApp
      ...
      // If order save fails, still open WhatsApp (don't block the sale)
    } catch {
      // Network error — still proceed with WhatsApp
    }

    const message = buildWhatsAppMessage(items, total, orderId);
